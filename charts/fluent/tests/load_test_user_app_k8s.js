...
const userActionRate = new Rate('user_actions');
const userErrorRate = new Rate('user_errors');

// 用户操作类型
const userActions = ['login', 'logout', 'register', 'profile_update', 'password_change', 'session_refresh'];
...
const FLUENT_BIT_USER_PORT = __ENV.FLUENT_BIT_USER_PORT || '8889';
...
export const options = {
  ...
  thresholds: {
    ...
    // 用户操作成功率应该大于 99%
    'user_actions': ['rate>0.99'],
  },
};

export default function () {
  // Kubernetes 环境下的 Fluent Bit Service URL
  const url = `http://${FLUENT_BIT_SERVICE}:${FLUENT_BIT_USER_PORT}/`;
  ...
  const message = logLevel === 'INFO' 
    ? `[USER] User ${action} successful - User ID: ${userId}, IP: ${ipAddress}`
    : `[USER] ${action} failed - User ID: ${userId}, IP: ${ipAddress}, Reason: Authentication error`;
...
export function setup() {
  ...
  console.log(`   Fluent Bit Service: ${FLUENT_BIT_SERVICE}:${FLUENT_BIT_USER_PORT}`);
  console.log(`   OpenSearch Service: ${OPENSEARCH_SERVICE}:${OPENSEARCH_PORT}`);
  if (isLocal) {
    console.log(`   ⚠️  确保已设置 port-forward: kubectl port-forward -n fluent svc/fluent-fluent-bit 8889:8889`);
  }
  console.log('');
  
  // 健康检查：验证 fluent-bit HTTP input 是否可用
  const healthUrl = `http://${FLUENT_BIT_SERVICE}:${FLUENT_BIT_USER_PORT}/`;
  const testPayload = JSON.stringify({
    message: '[USER] Health check message',
    level: 'INFO',
    user_id: 'USER-HEALTH',
    action: 'health_check',
  ...
  if (healthRes.status !== 200 && healthRes.status !== 201) {
    ...
    if (isLocal) {
      ...
      console.error('   kubectl port-forward -n fluent svc/fluent-fluent-bit 8889:8889');
    } else {
      console.error('   请确保 fluent-bit-sidecar 在 Kubernetes 中运行');
    }
    console.error(`   Status: ${healthRes.status}, Response: ${healthRes.body}`);
    throw new Error(`Fluent-bit HTTP input (User App) is not available. Status: ${healthRes.status}`);
  }
  
  console.log('✅ Health check passed. Fluent-bit HTTP input (User App) is ready.');
  console.log(`🚀 Starting User App load test (${isLocal ? 'Local Port-Forward' : 'Kubernetes Pod'})...`);
...
export function teardown(data) {
  ...
  try {
    ...
    // 查询正常日志
    const normalLogsUrl = `${opensearchBaseUrl}/user-logs-*/_count?q=service_name:user-app`;
    ...
    if (normalLogsRes.status === 200) {
      ...
      console.log(`\n✅ Normal logs in OpenSearch (user-logs-*): ${normalCount.toLocaleString()}`);
    }
    
    // 查询错误日志
    const errorLogsUrl = `${opensearchBaseUrl}/user-error-logs-*/_count?q=service_name:user-app`;
    const errorLogsRes = http.get(errorLogsUrl, {
      timeout: '10s',
    });
    if (errorLogsRes.status === 200) {
      const errorCount = JSON.parse(errorLogsRes.body).count;
      console.log(`⚠️  Error logs in OpenSearch (user-error-logs-*): ${errorCount.toLocaleString()}`);
    }
    
    // 查询最近的日志示例
    const recentLogsUrl = `${opensearchBaseUrl}/user-logs-*/_search?q=service_name:user-app&size=1&sort=@timestamp:desc`;
    ...
    if (recentLogsRes.status === 200) {
      ...
      if (recentLogs.hits && recentLogs.hits.hits.length > 0) {
        ...
        console.log(`\n📝 Latest log sample:`);
        console.log(`   User ID: ${latestLog.user_id || 'N/A'}`);
        console.log(`   Action: ${latestLog.action || 'N/A'}`);
        console.log(`   Level: ${latestLog.log_level || 'N/A'}`);
        console.log(`   IP: ${latestLog.ip_address || 'N/A'}`);
