...
export function setup() {
  ...
  const healthUrl = 'http://localhost:8889/';
  ...
  console.log('🚀 Starting User App load test...');
...
export function teardown(data) {
  ...
  try {
    // 查询正常日志
    const normalLogsRes = http.get('http://localhost:9200/user-logs-*/_count?q=service_name:user-app', {
      timeout: '10s',
    });
    ...
    // 查询错误日志
    const errorLogsRes = http.get('http://localhost:9200/user-error-logs-*/_count?q=service_name:user-app', {
      timeout: '10s',
    });
    ...
    // 查询最近的日志示例
    const recentLogsRes = http.get('http://localhost:9200/user-logs-*/_search?q=service_name:user-app&size=1&sort=@timestamp:desc', {
      timeout: '10s',
    });
